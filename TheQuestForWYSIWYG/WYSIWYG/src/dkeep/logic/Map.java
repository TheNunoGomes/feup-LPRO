	...
	private void startMap() {
		...
		for(row=0;row<10;row++) {
			for(column=0;column<10;column++) {
				...
				gameMap[row][column]= new Element(newPos,' ',this);
			}
		}
		for(row=0;row<10;row++) {
			gameMap[row][0]=new Element(new int[] {row,0},'X',this);
			gameMap[row][9]=new Element(new int[] {row,9},'X',this);
		}
		for(column=0;column<10;column++) {
			gameMap[0][column]=new Element(new int[] {0,column},'X',this);
			gameMap[9][column]=new Element(new int[] {9,column},'X',this);
		}
		for(row=2;row<8;row++) {
			for(column=2;column<8;column++) {
				if(column == 4 || column == 6 || row == 5) continue;
				else gameMap[row][column] = new Element(new int[] {row,column},'X',this);
			}
		}
		gameMap[5][7] = new Element(new int[] {5,7},'X',this);
		gameMap[8][2] = new Element(new int[] {8,2},'X',this);
		gameMap[8][3] = new Element(new int[] {8,3},'X',this);
		gameMap[5][9] = new Element(new int[] {5,9},'E',this);
	}
